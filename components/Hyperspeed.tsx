...
const DEFAULT_OPTIONS: EffectOptions = {
  onSpeedUp: () => {},
  onSlowDown: () => {},
...
export default function Hyperspeed({ effectOptions = {} }: { effectOptions?: EffectOptions }) {
  ...
  const stableOptions = useMemo(() => effectOptions, []);
    ...
    const distortions: Record<string, any> = {
      mountainDistortion: {
        ...
        getJS: (progress: number, time: number) => {
          const fix = 0.02, uF = mountainUniforms.uFreq.value, uA = mountainUniforms.uAmp.value;
      ...
      xyDistortion: {
        ...
        getJS: (progress: number, time: number) => {
          const fix = 0.02, uF = xyUniforms.uFreq.value, uA = xyUniforms.uAmp.value;
          const d = new THREE.Vector3(
            Math.cos(progress*Math.PI*uF.x+time)*uA.x - Math.cos(fix*Math.PI*uF.x+time)*uA.x,
            Math.sin(progress*Math.PI*uF.y+time+Math.PI/2)*uA.y - Math.sin(fix*Math.PI*uF.y+time+Math.PI/2)*uA.y,0
          );
      ...
      LongRaceDistortion: {
        ...
        getJS: (progress: number, time: number) => {
          const cam = 0.0125, uF = LongRaceUniforms.uFreq.value, uA = LongRaceUniforms.uAmp.value;
          const d = new THREE.Vector3(
            Math.sin(progress*Math.PI*uF.x+time)*uA.x - Math.sin(cam*Math.PI*uF.x+time)*uA.x,
            Math.sin(progress*Math.PI*uF.y+time)*uA.y - Math.sin(cam*Math.PI*uF.y+time)*uA.y,0
          );
...
${THREE.ShaderChunk['fog_fragment']}
    ...
    const roadMarkings_fragment = `
      ...
      float laneWidth=1.0/uLanes; float brokenLineWidth=laneWidth*uBrokenLinesWidthPercentage;
...
${THREE.ShaderChunk['fog_fragment']}
    ...
    class LightsSticks {
      ...
      init() {
        ...
        for (let i = 0; i < totalSticks; i++) {
          ...
          aMetrics.push(random(options.lightStickWidth) as number, random(options.lightStickHeight) as number);
        }
    ...
    class App {
      options: any; container: HTMLElement; hasValidSize: boolean; renderer: THREE.WebGLRenderer;
      composer: EffectComposer; camera: THREE.PerspectiveCamera; scene: THREE.Scene;
      ...
      constructor(container: HTMLElement, options: any) {
        ...
        // ensure canvas fills container regardless of attribute size
      ...
      dispose() {
        ...
        this.scene?.traverse((object: any) => {
          ...
          if (object.material) { Array.isArray(object.material) ? object.material.forEach((m: any) => m.dispose()) : object.material.dispose(); }
        });
        this.scene?.clear();
    ...
    const mergedOptions = { ...DEFAULT_OPTIONS, ...stableOptions, colors: { ...DEFAULT_OPTIONS.colors, ...stableOptions.colors } };
    (mergedOptions as any).distortion = distortions[mergedOptions.distortion as string] || { uniforms: distortion_uniforms, getDistortion: distortion_vertex };
    ...
    try {
      ...
      // Watch for container size changes (initial 0×0, hero text reflow, window resize, etc.)
      if (typeof ResizeObserver !== 'undefined') {
        resizeObserver = new ResizeObserver(() => {
          if (appRef.current?.onWindowResize) appRef.current.onWindowResize();
        });
