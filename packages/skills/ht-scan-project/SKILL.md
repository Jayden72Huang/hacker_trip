...
**重要**：API 返回的数据包含 `slug` 字段，用于生成平台链接：`https://hackertrip.space/hackathon/{slug}`（网站同时支持 UUID 和 slug 路由）。优先使用 API 数据。
...
使用 markdown 格式输出，不使用 box-drawing 字符（中英文混排在终端中无法对齐右边框）：
