...
Page({
  data: {
    ...
    // 表单
    techStack: ['Next.js', 'TypeScript', 'Claude SDK'],
    aiTools: ['Claude Code'],
    playStyleIdx: 0,
    lookingForIdx: 1,
    techInput: '',
    ...
    // 统计（可由同步数据带入）
    stats: { projects: 6, hackathons: 4, awards: 1 },
    saving: false,
    canvasReady: false,
    fromSync: false,
  },
  ...
  onLoad() {
    ...
    if (scan && scan.identity) {
      ...
      this.setData({
        ...
        stats: { projects: id.projects || 6, hackathons: id.hackathons || 4, awards: id.awards || 1 },
        fromSync: true,
      });
    }
    this.recompute();
